...
<div class="layui-layout layui-layout-admin">
    <div class="layui-header">
        ...
        <ul class="layui-nav layui-layout-left">
            ...
            <li class="layui-nav-item layui-this"><a href="Library_mylend.php">我的图书</a></li>
    ...
    <div class="layui-body">
        ...
        <div style="margin: 20px 50px 0 50px;">
            ...
            <script type="text/html" id="barDemo">
                ...
                <a class="layui-btn layui-btn-xs" lay-event="addlend">续借</a>
                <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="back">还书</a>
            </script>
        </div>
    </div>
...
<script>
    ...
    layui.use(['element','form','jquery','layer','table','laypage'], function(){
        ...
        table.render({
            elem: '#demo'
            ,height: 750
            ,url: './php/bookjson3.php' //数据接口
            ,id: 'table_mylend'
            ,toolbar: '<div>' +
                '<div class="layui-inline" lay-event="search" title="搜索"><i class="layui-icon layui-icon-search"></i></div>'+
                '<div class="layui-inline" lay-event="addlend" title="续借"><i class="layui-icon layui-icon-add-circle"></i></div>' +
                '<div class="layui-inline" lay-event="back" title="还书"><i class="layui-icon layui-icon-reduce-circle"></i></div>' +
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'lendtime', title: '剩余时间（天）', width: 150,sort: true}
        ...
        var searchHtml = ' <div class="layui-form-item" style="margin-left: 10px;margin-top: 30px;">\n' +
            ...
            '</div>'
        //续借框html代码
        var addLendHtml = '<div class="layui-form" style="margin:30px 0 0 25px;">\n' +
            '                <div class="layui-form-item">\n' +
            '                    <label class="layui-form-label">续借天数</label>\n' +
            '                    <div class="layui-input-block" style="width: 100px;">\n' +
            '                        <select name="day" id="day" lay-verify="required">\n' +
            ...
            switch(obj.event){
                    ...
                    layer.open({
                        ...
                        ,yes: function(index, layero){ //点击搜索按钮的回调
                            if($('#searchtext').val()==''){
                                ...
                                table.reload('table_mylend', {
                                    url: './php/usertable_search3.php'
                                    ,where: {
                    ...
                    if(data.length === 0){
                        ...
                        if (data.length > 0) {
                               layer.open({
                                    ...
                                    ,yes: function (index, layero) { //点击搜索按钮的回调
                                        if ($('#day').val() == '') {
                                            layer.msg('请选择续借天数！', {icon: 2});
                                            return false;
                                        } else {
                                            var day = $('#day').val();
                                            table.reload('table_mylend', {
                                                url: './php/usertable_addlend.php'
                                                , where: {
                                                    book_id: arr,
                                                    lend_day: day
                                                } //设定异步数据接口的额外参数
                ...
                case 'back':
                    if(data.length === 0){
                        ...
                        if (data.length > 0) {
                            layer.confirm('确定归还选中的书籍（此操作将不可撤销）？', {icon: 3, title: '提示信息'}, function (index) {

                                layer.close(index);
                                table.reload('table_mylend', {
                                    url: './php/usertable_back.php'
                                    ,where: {
                                        book_id: arr
                                    } //设定异步数据接口的额外参数
                                    ,done: function(res, curr, count){
                                        layer.msg('还书成功，你可以继续借阅！', {icon: 1});
                                        table.reload('table_booklend', {
                                           url: './php/bookjson3.php'
                                           ,done:function () {
        ...
        table.on('tool(test)', function(obj){
            ...
            if(layEvent === 'detail') {
                layer.tab({
                    ...
                    tab: [{
                        ...
                        content: '<p style="text-align: center;font-size:20px;padding-top:10px; ">ID：' + editList[0] + '</p>' +
                            ...
                            '<p style="text-align: center;font-size:20px;padding-top:10px;">剩余时间：' + editList[7] + '天' + '</p>'
                    }, {
                        title: '图书封面',
                ...
                layer.confirm('确定归还此书籍（此操作将不可撤销）？', {icon: 3, title: '提示信息'}, function(index){
