...
//查询数据库是否有存在该用户
    $exist = mysqli_query($con,"SELECT * FROM librarycms_login WHERE uname = '$user'");
    $exist_result = mysqli_num_rows($exist);
    if($exist_result){
        //如果存在该用户
        echo 1;
    }else{
        mysqli_query($con,"ALTER TABLE librarycms_login auto_increment=1");
        //插入数据库
        mysqli_query($con,"INSERT INTO librarycms_login (uname, pwd) VALUES ('$user', '$psw')");
        echo 2;
    }
