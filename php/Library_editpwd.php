...
//依据用户名更新数据库密码字段
    ...
    //查询数据库账号密码是否一致
    $exist = mysqli_query($con,"SELECT * FROM library_login WHERE uname = '".$_SESSION['username1']."'");
    $exist_result = mysqli_num_rows($exist);
    if($exist_result){
        mysqli_query($con,"update library_login set pwd='$psw' where uname='".$_SESSION['username1']."'");
        echo 3;
    }else{
        echo 4;
    }
